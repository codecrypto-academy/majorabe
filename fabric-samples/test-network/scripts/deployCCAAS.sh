...
packageChaincode() {
  ...
  # Versión para usar chaincode como contenedor en Docker (por defecto de Fabric):
  # address="{{.peername}}_${CC_NAME}_ccaas:${CCAAS_SERVER_PORT}"
  # ↑ Esta dirección apunta al nombre del contenedor del chaincode, válido solo si el chaincode corre dentro de la red Docker.

  # Versión para usar chaincode como servicio externo (ej. ejecutado con "npm start" en la máquina local):
  address="host.docker.internal:${CCAAS_SERVER_PORT}"
  # ↑ Esta dirección permite que los peers (en Docker) se conecten al chaincode corriendo fuera de Docker (en el host).
  #    Docker no puede resolver "localhost" como tu máquina host, pero "host.docker.internal" sí funciona en Linux con WSL, Mac y Windows.
  
  #  Windows con WSL, a veces host.docker.internal no es resolvible desde dentro del contenedor del peer
  #  Solucion: usar la IP del host WSL2 en lugar de host.docker.internal.Encuentra esta ip ejecutando:  ip addr | grep eth0
  # address="172.18.229.112:${CCAAS_SERVER_PORT}"
