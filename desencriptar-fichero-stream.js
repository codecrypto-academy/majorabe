...
fs.createReadStream(inputFile) // Crea un flujo de lectura desde el archivo cifrado de entrada.
    .pipe(descifrador) // Aplica el proceso de descifrado utilizando la instancia del descifrador AES.
    .pipe(fs.createWriteStream(outputFile)) // Escribe los datos descifrados en el archivo de salida.
    .on('finish', () => console.log('Archivo descifrado con éxito.')) // Mensaje de confirmación al completar el proceso.
    .on('error', (err) => console.error('Error durante el descifrado:', err.message)); // Manejo de errores durante el flujo de datos.
