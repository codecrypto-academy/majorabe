...
## 1. Configuración de MySQL🛠️
1.1 Creo el contenedro con Mysql
```bash
docker run --name curso-pg -e POSTGRES_PASSWORD=xxx -p 5437:5432 -d postgres:13
```
...
## 2. Configuración de Postgresql🛠️
2.1 Creo el contenedor
```bash
docker run --name proyPostgres -e POSTGRES_PASSWORD=xxx -p 5433:5432 -d postgres:13 
```
...
## 3. Configuración de SQLServer🛠️
3.1 Creo el contenedor 
```bash
docker run --name sqlServer -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=xxx" -p 1433:1433 -d mcr.microsoft.com/mssql/server:2019-CU15-ubuntu-20.04
```
...
## 4. Configuración de Oracle🛠️
4.1 Creo el contenedor
```bash
docker run -d --name oracle -e ORACLE_PWD=xxx -p 1521:1521 container-registry.oracle.com/database/express:21.3.0-xe
```
